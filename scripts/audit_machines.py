...
def machine_paths() -> list[Path]:
    roots = [Path("kb/items"), Path("kb/imports")]
    paths: list[Path] = []
    for root in roots:
        if root.exists():
...
def is_active_ebf(machine_id: str, data: dict[str, Any], path: Path) -> bool:
    text = " ".join([machine_id, str(data.get("name", "")), str(path)]).lower()
    return "ebf" in text or "ebam" in text
...
def collect_resource_uses() -> dict[str, list[dict[str, str]]]:
    ...
    for path in Path("kb/processes").glob("*.yaml"):
        data = load_yaml(path)
...
def audit(apply: bool) -> None:
    ...
    for machine_id, data, path in machines:
        active_ebf = is_active_ebf(machine_id, data, path)
        ...
        if not active_ebf:
            ...
            if str(path).startswith("kb/items/parts/"):
                audit_tags.append("machine_audit_misplaced_file")
            if str(path).startswith("kb/imports/") or data.get("is_import") is True:
        ...
        if mismatches and not active_ebf:
            audit_tags.append("machine_audit_semantic_mismatch")

        final_tags = non_audit_tags + [tag for tag in sorted(set(audit_tags)) if tag not in non_audit_tags]
        ...
        rows.append(
            {
                ...
                "path": str(path),
                "active_ebf_excluded": active_ebf,
                "audit_tags": ";".join(audit_tags),
                ...
                "risk_flags": ";".join(sorted(set(audit_tags))),
    ...
    clean_count = sum(1 for row in rows if not row["audit_tags"] and not row["active_ebf_excluded"])
    ebf_count = sum(1 for row in rows if row["active_ebf_excluded"])
    md_lines = [
        ...
        f"- Clean non-EBF machines: {clean_count}",
        f"- Active EBF/EBAM machines excluded from automatic tagging: {ebf_count}",
        "",
        "## Audit Tag Counts",
        "",
    ]
    ...
    md_lines.extend(
        [
            ...
            "item/process-specific review before being used for EBF nominal local routes.",
            "",
        ]
    )
    (out_dir / "README.md").write_text("\n".join(md_lines), encoding="utf-8")

    print(f"machines={len(rows)} changed={changed} clean_non_ebf={clean_count} ebf_excluded={ebf_count}")
    for tag, count in tag_counts.most_common():
        print(f"{tag}: {count}")
