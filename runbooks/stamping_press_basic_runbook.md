...
- cmd: sim.note
  args:
    style: milestone
    message: "Assemble stamping_press_basic from imported parts."
- cmd: sim.run-recipe
  args:
...
- cmd: sim.note
  args:
    style: success
    message: "Baseline stamping_press_basic complete."
```
...
- cmd: sim.note
  args:
    style: milestone
    message: "Stage 2: ISRU production run (regolith metal + select subcomponents)."
- cmd: sim.note
  args:
    style: info
    message: "Import baseline equipment and remaining imported components."
- cmd: sim.import
  args:
    item: labor_bot_general_v0
    quantity: 2
