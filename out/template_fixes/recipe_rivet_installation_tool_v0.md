# Fix Intelligence: recipe_rivet_installation_tool_v0

## Files

- **Recipe:** `kb/recipes/recipe_rivet_installation_tool_v0.yaml`
- **Target item:** `rivet_installation_tool_v0`
  - File: `kb/items/rivet_installation_tool_v0.yaml`
- **BOM:** `kb/boms/bom_rivet_installation_tool_v0.yaml` ✓
  - Components: 3
- **Steps:** 3 total

## Errors (3 found)

### Error 1: recipe_template_missing_step_inputs

**Message:** Step 0 uses template process 'metal_casting_basic_v0' but doesn't provide step-level input overrides

**Location:** Step 0
**Process:** `metal_casting_basic_v0`
  - File: `kb/processes/metal_casting_basic_v0.yaml`

**Process type:** TEMPLATE (requires step-level inputs)

**Current step:**
```yaml
- process_id: metal_casting_basic_v0
  # NO inputs field
```

**Analysis:** Template process used without step-level input overrides.

#### Option A: Use BOM components

BOM has 3 components:

- `steel_plate_or_sheet` (qty: 4.0 kg)
- `spring_compression_small` (qty: 2 unit)
- `fastener_kit_small` (qty: 1 unit)

Suggested fix:
```yaml
- process_id: metal_casting_basic_v0
  inputs:
  - item_id: steel_plate_or_sheet
    qty: 4.0
    unit: kg
  - item_id: spring_compression_small
    qty: 2
    unit: unit
  - item_id: fastener_kit_small
    qty: 1
    unit: unit
```

---

### Error 2: recipe_template_missing_step_inputs

**Message:** Step 1 uses template process 'machining_finish_basic_v0' but doesn't provide step-level input overrides

**Location:** Step 1
**Process:** `machining_finish_basic_v0`
  - File: `kb/processes/machining_finish_basic_v0.yaml`

**Process type:** TEMPLATE (requires step-level inputs)

**Current step:**
```yaml
- process_id: machining_finish_basic_v0
  # NO inputs field
```

**Analysis:** Template process used without step-level input overrides.

#### Option A: Use BOM components

BOM has 3 components:

- `steel_plate_or_sheet` (qty: 4.0 kg)
- `spring_compression_small` (qty: 2 unit)
- `fastener_kit_small` (qty: 1 unit)

Suggested fix:
```yaml
- process_id: machining_finish_basic_v0
  inputs:
  - item_id: steel_plate_or_sheet
    qty: 4.0
    unit: kg
  - item_id: spring_compression_small
    qty: 2
    unit: unit
  - item_id: fastener_kit_small
    qty: 1
    unit: unit
```

#### Option B: Use previous step outputs

- Step 0 produces: `cast_metal_parts` (0.95 kg)

---

### Error 3: recipe_template_missing_step_inputs

**Message:** Step 2 uses template process 'assembly_basic_v0' but doesn't provide step-level input overrides

**Location:** Step 2
**Process:** `assembly_basic_v0`
  - File: `kb/processes/assembly_basic_v0.yaml`

**Process type:** TEMPLATE (requires step-level inputs)

**Current step:**
```yaml
- process_id: assembly_basic_v0
  # NO inputs field
```

**Analysis:** Template process used without step-level input overrides.

#### Option A: Use BOM components

BOM has 3 components:

- `steel_plate_or_sheet` (qty: 4.0 kg)
- `spring_compression_small` (qty: 2 unit)
- `fastener_kit_small` (qty: 1 unit)

Suggested fix:
```yaml
- process_id: assembly_basic_v0
  inputs:
  - item_id: steel_plate_or_sheet
    qty: 4.0
    unit: kg
  - item_id: spring_compression_small
    qty: 2
    unit: unit
  - item_id: fastener_kit_small
    qty: 1
    unit: unit
```

#### Option B: Use previous step outputs

- Step 0 produces: `cast_metal_parts` (0.95 kg)
- Step 1 produces: `machined_part_raw` (1.0 kg)

---

## Summary

- **Total errors:** 3
- **Recipe file:** `kb/recipes/recipe_rivet_installation_tool_v0.yaml`
- **BOM available:** Yes (3 components)
