...
New parts should state what they are actually made from with `material`. This is
a property of the item itself, not a substitution rule. Existing
`material_class` fields remain readable while each content repository migrates.
...
For KB item classification, use `kind: raw_material` only for natural or primary
source materials and `kind: material` for process-ready feedstocks,
intermediates, stock forms, byproducts, and scrap. The authoritative lunar
source/material/resource distinction is maintained in
`docs/lunar_material.md`.
...
Do not use `material_class` as new substitution evidence. New part items should
...
metadata. Legacy entries can retain `material_class` until reviewed.
...
|-----------|------|-------------|-------|
| `labor_bot_general_v0` | 120 kg | Assembly, quality control, material handling, mechanical/electrical assembly | 6-DOF industrial manipulator, 2m reach, 20kg payload, +/-0.5mm repeatability. Detailed specs in `content/kb/items/machines/labor_bot_general_v0.yaml`. |
...
2. **Search existing parts** (MANDATORY)
   ...
   grep -r "material: steel" content/kb/items/parts/
   ```
