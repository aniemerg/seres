# Lunar Material Availability and Resource Classification Guide

Date: 2026-07-31

Decision record: `docs/ADRs/028-material-raw-material-resource-classification.md`.

Purpose: define the KB distinction between lunar raw source materials,
process-ready materials, and non-material resources, and guide material choices
when adding new lunar machine designs to the KB. This is a screening guide, not
proof of site availability or production closure. A material being
present on the Moon does not mean the current KB can already close a local
manufacturing path to it.

Evidence boundary: the availability tiers summarize project research and are
design heuristics. Before a material controls a simulation conclusion, verify
the exact source, site assumption, and process route. Relevant source texts are
kept under `design/papers/`, including the Chirikjian lunar-factory studies,
Ellery self-replication studies, and the NSS lunar-industry study.

## Formal KB Classification

Use these item classes consistently:

| Class | Directory | Kind | Definition | Examples |
|---|---|---|---|---|
| Raw source material | `content/kb/items/raw_materials/` | `raw_material` | Natural, mined, collected, or otherwise primary source material that is not yet an engineering material. It usually needs beneficiation, separation, refining, crushing, or other preparation before use in normal manufacturing recipes. | `regolith_lunar_highlands`, `regolith_lunar_mare`, `anorthite_ore`, `plagioclase_feldspar`, `nife_meteorite_material` |
| Process-ready material | `content/kb/items/materials/` | `material` | Engineering material, chemical, feedstock, stock form, powder, wire, ingot, slurry, intermediate, byproduct, or scrap that a process can intentionally consume or produce. It may still be crude or low-purity, but it is already a defined process material rather than an environmental source. | `steel_stock`, `alumina_powder`, `ceramic_powder`, `regolith_powder`, `ilmenite_concentrate`, `metal_wire_feed` |
| Non-material resource | `content/kb/items/resources/` | `resource` | Energy, environmental flux, information-like resource, or other non-mass resource used by recipes/processes. These are not materials and should not be placed in `raw_materials`. | `solar_irradiance`, `solar_radiation`, `concentrated_solar_thermal` |

The decision rule is:

- If it is a natural/source-side input from the Moon, a meteorite, a volatile
  deposit, or another primary environment, classify it as `raw_material`.
- If it is a prepared material state usable by a manufacturing process, classify
  it as `material`.
- If it is energy, radiation, heat, data, compute, or another non-material
  service/flux, classify it as `resource`.

Do not classify a powder or concentrate as `raw_material` merely because it came
from regolith. Once a recipe has produced `regolith_powder`,
`ilmenite_concentrate`, `olivine_concentrate`, `pyroxene_concentrate`, or another
controlled feedstock, it is a `material`.

Imported raw ores or imported source placeholders may still be `raw_material`
when their identity is an ore/source item rather than a process-ready engineering
material. Use `is_import: true` to mark the supply boundary.
...
material to `content/kb/items/raw_materials/` (`kind: raw_material`) or to an explicit
import, the design has an unproven supply path in the current KB.
...
|---|---|---|
| `local_raw` | Direct in-situ source item exists in `content/kb/items/raw_materials/` | Good baseline input. |
...
1. Search for an existing machine, tool, part, or material with equivalent
   function, material requirements, and scale.
