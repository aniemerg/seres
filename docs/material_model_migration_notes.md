...
2. Create a canonical material list, likely under `kb/materials/` or another
   dedicated directory.
3. Create explicit material substitution groups, with context limits.
4. Change validators so parts prefer or require `material`, not
   `material_class`.
...
Initial migration has started:

- `docs/kb_schema_reference.md` lists `part.material` as required.
- `src/kb_core/schema.py` supports `material` on raw and strict item models.
- `src/indexer/indexer.py` reports missing `material` on parts instead of
  missing `material_class`.
- Existing part files have been migrated from top-level `material_class` to
  top-level `material`.
- Machine and material item files have had top-level `material_class` removed.
  Machines describe composition through BOM components; material items are
  themselves materials and should use fields like composition, state, density,
  and future material groups.
- Any remaining code fallback for `material_class` is compatibility-only.

The generated audit files under `out/material_audit/` list broad or functional
part material values that still need human review.
