...
performance_requirements:
  tolerance: "+/-0.01 mm bearing seat"
  surface_finish: "Ra <= 0.4 um on sealing face"
  sealing_quality: "helium leak test required; target leak rate TBD"
  alignment_accuracy: "coaxiality budget TBD"
```

Rules:

- `performance_requirements` is optional.
- Populate only fields that matter for the item's role.
- Leave unrelated requirements absent instead of adding null placeholders.
- Values may initially be concise strings when the exact numeric requirement is
  unknown.
- Use `review_required` only when the requirement is known to matter but the
  numeric value is not yet available.
- Additional requirement keys may be added later for domains such as electrical
  rating, thermal load, optical clarity, radiation tolerance, or software
  behavior.
...
- Some early values will be coarse strings until stronger sources are available.
...
Future validation should add warnings when:

- `mass_low_kg > mass_high_kg`,
- `mass_kg` is outside the declared low/high range,
- `performance_requirements` is present but not a mapping,
- a field is set to an empty string or null placeholder.
