...
**Related ADRs:** ADR-012 (Process Types and Time Model), ADR-013 (Recipe Overrides), ADR-016 (Unit Conversion)

## Context

The knowledge base has energy models in many processes, but the current schema has the same fundamental limitations as the time_model schema addressed in ADR-012:
...
We will implement a **new energy_model schema** that parallels the time_model redesign (ADR-012):
...
1. **Parallel to time model** - Same migration effort and timeline as ADR-012
2. **Recipe impacts** - Recipe override mechanics need specification (ADR-013)
3. **Unit conversion** - Requires implicit conversion system (ADR-016)
...
- Parallel design: ADR-012 (Process Types and Time Model Redesign)

## Related Decisions

- **ADR-012:** Time model redesign - parallel structure
- **ADR-013:** Recipe override mechanics (est_energy_kWh → energy_model overrides)
