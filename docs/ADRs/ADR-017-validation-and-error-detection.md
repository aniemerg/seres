...
**Related ADRs:** ADR-012 (Process Types), ADR-014 (Energy Model), ADR-016 (Unit Conversion), ADR-013 (Recipe Overrides)

## Context

The knowledge base has grown to ~891 processes, hundreds of recipes, thousands of items, and extensive interconnected data. With the new schemas from ADR-012, ADR-014, and ADR-016, we need a comprehensive validation strategy to:
...
With ADR-012, ADR-014, and ADR-016, we now need:
...
**Purpose:** Ensure unit conversions are possible (ADR-016 integration)
...
**Purpose:** Ensure recipe overrides are valid (ADR-013 integration)
...
- ADR-012: Process types and time model (validation rules lines 93-113)
- ADR-014: Energy model redesign (validation rules)
- ADR-016: Unit conversion system (conversion validation)
- ADR-013: Recipe override mechanics (override validation)
- Current indexer: `kbtool/indexer.py`
- User feedback: `design/time-model-hierarchy-feedback.txt`

## Related Decisions

- **ADR-012:** Time model schema defines validation rules
- **ADR-014:** Energy model schema defines validation rules
- **ADR-016:** Unit conversion defines convertibility validation
- **ADR-013:** Recipe overrides define override validation
