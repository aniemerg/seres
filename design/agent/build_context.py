...
Outputs:
- design/agent/cached_context.md - Static context (memos + KB structure + examples + papers)
...
OUTPUT_FILE = REPO_ROOT / "design" / "agent" / "cached_context.md"
...
def build_context() -> str:
    ...
    # Section 1: Project Memos
    sections.append("---\n## 1. Project Memos\n")

    sections.append("### Meta-Memo (Project Overview)\n")
    meta_memo = read_file(DESIGN_ROOT / "meta-memo.md")
    sections.append(meta_memo)

    sections.append("\n### Memo A (Software Specification)\n")
    memo_a = read_file(DESIGN_ROOT / "memo_a.md")
    sections.append(memo_a)

    sections.append("\n### Memo B (Knowledge Acquisition Protocol)\n")
    memo_b = read_file(DESIGN_ROOT / "memo_b.md")
    sections.append(memo_b)
