...
Usage:
    python -m design.agent.worker --agent <agent_name> [--model <model>] [--max-items <n>]
"""
...
def build_agent(model: str = "gpt-5-nano") -> Agent:
    ...
    from design.agent.kb_tools import rg_search, read_file, write_file, run_indexer, queue_release
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            try:
                ...
                if iteration == 1:
                    ...
                    result = asyncio.run(run_agent_streamed(agent, input_with_lease, max_turns=20))
                else:
                    # Subsequent iterations: agent continues with feedback
                    result = asyncio.run(run_agent_streamed(agent, user_input, max_turns=20))
