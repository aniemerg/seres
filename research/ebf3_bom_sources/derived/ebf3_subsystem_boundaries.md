...
| `ebf3_high_voltage_input` | Fixed electron beam gun | Interface between HV tank/cable and gun. | Keep in electron gun as gun-side receiving/feedthrough hardware. | If decomposed, split into central conductor, ceramic insulator, flange, and cable-side termination. |
...
| `ebf3_lens_corrector_current_supplies` | Power supplies | Supplies drive gun magnetic lenses/correctors. | Keep in power supplies; coils remain in gun. | Add explicit load references later if schema supports electrical interfaces. |
| `ebf3_power_supply_control_board` | Power supplies | Could be mistaken for central controls. | Keep in power supplies if it is an internal PSU controller. | Rename or note as "power-supply internal control board" if ambiguity causes review friction. |
...
| `ebf3_process_monitor_lighting` | Controls | Physical lights may mount to cabin. | Keep in controls for instrumentation package. | Split lamp fixture mount/port into cabin later if needed. |
| `ebf3_cabin_feedthroughs_and_wiring_ports` | Manufacture cabin | Can duplicate feeder, positioning, gun, or sensor feedthroughs. | Keep as generic chamber-side ports/flanges only. | Do not use as a substitute for subsystem-specific feedthrough inserts. |
| `ebf3_wire_feeder_feedthrough_connector` | Wire feeder | Penetrates chamber boundary. | Keep feeder-specific connector insert in wire feeder. | Cabin should own only mating chamber port/flange. |
...
| `ebf3_wire_feeder_mount_to_gun_bracket` | Wire feeder | Mechanical interface to electron gun. | Keep in wire feeder if removable feeder package includes the bracket. | If bracket is integral to gun column, split gun-side mounting datum into electron gun. |
...
| `ebf3_beam_current_return_strap` | Four-axis positioning system | Electrical return path may relate to power/HV. | Keep with platform/substrate path for now. | Re-audit after source review; may need split into platform strap and cabinet return conductor. |
